  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      pool.getConnection(function(err, connection) {
        connection.query('SELECT equip_id FROM equip_mastr', function(err, rows) { // 시스템 내 전체 설비 목록을 조회하는 쿼리
    ...
    equipMapping(equipList[0]); // ************해당 부분 for문으로 처리해야 함**********
...
function equipMapping(equip_id) {
  ...
  async.parallel([
    ...
    function(callback){
      console.log('parallel first function start');
      pool.getConnection(function(err, connection) {
        connection.query("SELECT * FROM mapng_equip_instm WHERE EQUIP_ID='" + equip_id + "'", function(err, rows) {
          ...
          for(var i in rows){
            ...
            console.log('mapng_equip_instmt_Object.instm_code : ', mapng_equip_instmt_Object.instm_code);
            mapng_equip_instmt_Array.push(mapng_equip_instmt_Object);
          }

          console.log('mapng_equip_instmt_Array : ', mapng_equip_instmt_Array);
          console.log('mapng_equip_instm table row : ', rows);

          // 컨넥션 반환
          connection.release();

          console.log('parallel first function end');
    ...
    function(callback){
      ...
      console.log('parallel second function start');
      ...
      pool.getConnection(function(err, connection) {
        ...
        connection.query("SELECT * FROM mapng_tssr_trgt WHERE EQUIP_ID='" + equip_id + "'", function(err, rows) {
          ...
          console.log('parallel second function end');
